...
class StreamingOpenWebTextDataset(IterableDataset):
    ...
    def __init__(self, tokenizer, max_seq_len: int = 1024, skip_samples: int = 0, rank: int = 0, world_size: int = 1):
        super().__init__()
    ...
    def __iter__(self) -> Iterator[Tuple[torch.Tensor, torch.Tensor]]:
        ...
        for sample in dataset:
            ...
            if samples_processed < self.skip_samples:
                samples_processed += 1
                continue

            # Distribute samples across ranks (each rank gets every Nth sample)
            if samples_processed % self.world_size != self.rank:
                samples_processed += 1
                continue

            tokens = self.tokenizer.encode(sample["text"], allowed_special="all")
...
def save_checkpoint(model, optimizer, scaler, step, samples_seen, model_args, output_dir, train_args, rank):
...
def load_checkpoint(path: str, model, optimizer, scaler):
    ...
    return checkpoint["step"], checkpoint["samples_seen"]
...
def train(args):
    ...
    if args.resume:
        ...
        start_step, samples_seen = load_checkpoint(args.resume, model, optimizer, scaler)
        print_main(f"[resume] Resuming from step {start_step}")

    # Dataloader (each rank gets different samples)
    dataset = StreamingOpenWebTextDataset(tokenizer, args.max_seq_len, samples_seen, rank, world_size)
    ...
    for input_ids, labels in dataloader:
        ...
        accum_count += 1
        samples_seen += input_ids.shape[0] * world_size  # Account for all ranks
        ...
        if accum_count >= args.grad_accum:
            ...
            if rank == 0:
                pbar.set_postfix({"loss": f"{accum_loss * args.grad_accum:.4f}", "lr": f"{lr:.2e}", "samples": samples_seen})
                pbar.update(1)
            ...
            if step > 0 and step % args.save_every == 0:
                ...
                if world_size > 1:
                    dist.barrier()
                save_checkpoint(model, optimizer, scaler, step, samples_seen, model_args, args.output_dir, args, rank)
