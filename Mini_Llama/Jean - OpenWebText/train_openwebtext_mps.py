...
class StreamingOpenWebTextDataset(IterableDataset):
    def __init__(self, tokenizer, max_seq_len: int = 1024, skip_samples: int = 0):
        super().__init__()
...
def save_checkpoint(model, optimizer, step, samples_seen, args, output_dir, train_args):
...
def train(args):
    ...
    if args.resume:
        ...
        start_step, samples_seen = load_checkpoint(args.resume, model, optimizer)
    ...
    # Dataloader
    dataset = StreamingOpenWebTextDataset(tokenizer, args.max_seq_len, samples_seen)
    ...
    for input_ids, labels in dataloader:
        ...
        if accum_count >= args.grad_accum:
            ...
            if step > 0 and step % args.save_every == 0:
                save_checkpoint(model, optimizer, step, samples_seen, model_args, args.output_dir, args)
